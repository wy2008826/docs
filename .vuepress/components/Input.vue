...
<script>
    ...
    import CellGroup from 'components/CellGroup/index';
    ...
    export default {
        ...
        methods: {
            ...
            confimPicker(value, i){//
                ...
                let item = (this.selects||[]).filter((item,i)=>JSON.stringify(item)==JSON.stringify(value));
...
<style lang="scss" scoped>
    ...
    .input{
        $h:1rem;
        $labelW:2rem;
        @include box((lh:$h,d:flex,bg:$white));
        ...
        .label{
            @include box((w:$labelW,h:$h,p:0 0 0 0.2rem));
            @include ellipsis();
        }
        input{
            @include box((d:block,h:$h,lh:normal,fs:0.28rem,bg:transparent));
            text-indent: 0.2rem;
            flex:1;
        }
        textarea{
            @include box((d:block,lh:0.6rem,fs:0.28rem,bg:transparent,p:0.2rem 0.2rem 0.1rem 0.2rem));
            min-height: 2rem;
            flex:1;
        }
        .switch{
        ...
        .arrow{
            @include box((d:block,w:1rem,p:0 0.2rem 0 0,ta:right,fs:0.3rem));
        }
        /deep/ .toast{
            z-index: 10;
        }
