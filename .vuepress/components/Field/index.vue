<template>
	...
	>
		...
		<textarea
			...
			:value="value"
		/>
		<input v-else v-bind="$attrs" v-on="listeners" :class="b('control')" :type="type" :value="value">
		<div v-if="errorMessage" v-text="errorMessage" :class="b('error-message')"/>
		<div v-if="showIcon" :class="b('icon')" @touchstart.prevent="onClickIcon">
			<slot name="icon">
				<icon :name="icon"/>
			</slot>
		</div>
